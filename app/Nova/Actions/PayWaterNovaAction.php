...
use App\Actions\PayWaterAction;
use Exception;
...
class PayWaterNovaAction extends Action
{
    ...
    public function handle(ActionFields $fields, Collection $models): ActionResponse|Action
    {
        ...
        $action = resolve(PayWaterAction::class);
        foreach ($models as $model) {
            try {
                if($model->is_paid) {
                    return Action::danger('Water already paid.');
                }
        ...
        return Action::redirect(route('print.water', $models->first()->id));
    }
